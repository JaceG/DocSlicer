	...
	for (let pageNum = 1; pageNum <= maxPages; pageNum++) {
		...
		try {
			...
			if (settings.mode === 'text') {
				...
				pageDiff.hasChanges =
					textDiff.additions > 0 ||
					textDiff.deletions > 0 ||
					textDiff.modifications > 0;
...
export async function generateComparisonReport(
	...
	y -= 40;
	summaryPage.drawText(`Document 1: ${file1.name}`, {
		x: leftMargin,
		y,
		size: 12,
		font,
	...
	y -= 20;
	summaryPage.drawText(`Document 2: ${file2.name}`, {
		x: leftMargin,
		y,
		size: 12,
		font,
	...
	y -= 20;
	summaryPage.drawText(`- Total pages compared: ${differences.length}`, {
		x: leftMargin + 20,
		y,
		size: 12,
		font,
	...
	y -= 18;
	summaryPage.drawText(`- Pages with changes: ${pagesWithChanges}`, {
		x: leftMargin + 20,
		y,
		size: 12,
		font,
	...
	summaryPage.drawText(
		`- Pages unchanged: ${differences.length - pagesWithChanges}`,
		{ x: leftMargin + 20, y, size: 12, font }
	);

	y -= 40;
	summaryPage.drawText('Page Details:', {
		x: leftMargin,
		y,
		size: 14,
		font: boldFont,
	...
	for (const diff of differences) {
		...
		const status = diff.hasChanges ? '[!] Changed' : '[OK] Unchanged';
		const changeText = diff.hasChanges
			? ` (${diff.changePercentage.toFixed(1)}% different)`
			: '';

		summaryPage.drawText(
			`Page ${diff.pageNumber}: ${status}${changeText}`,
			{
				x: leftMargin + 20,
				y,
				size: 11,
