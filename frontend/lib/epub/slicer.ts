...
import { SliceTask, SliceResult } from '@/types';
  ...
  <navMap>
    ...
    <navPoint id="navpoint-${index + 1}" playOrder="${index + 1}">
      ...
      <content src="chapter_${startChapter + index + 1}.xhtml"/>
		...
		// Store in blob store
		const blobKey = `epub-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
		...
		return {
			success: false,
			error: error instanceof Error ? error.message : 'Failed to slice EPUB',
		};
	}
}
	...
	for (const task of tasks) {
		...
		try {
			...
			const result = await sliceEPUB(file, {
				startChapter: task.startPage - 1, // Convert to 0-based index
				endChapter: task.endPage - 1,
